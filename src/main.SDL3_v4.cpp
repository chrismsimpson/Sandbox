...
public:
    ...
    {
        return Vec4(
            i.x() * m.m_matrix[0][0] + i.y() * m.m_matrix[1][0] + i.z() * m.m_matrix[2][0] + i.w() * m.m_matrix[3][0],
            i.x() * m.m_matrix[0][1] + i.y() * m.m_matrix[1][1] + i.z() * m.m_matrix[2][1] + i.w() * m.m_matrix[3][1],
            i.x() * m.m_matrix[0][2] + i.y() * m.m_matrix[1][2] + i.z() * m.m_matrix[2][2] + i.w() * m.m_matrix[3][2],
            i.x() * m.m_matrix[0][3] + i.y() * m.m_matrix[1][3] + i.z() * m.m_matrix[2][3] + i.w() * m.m_matrix[3][3]);
    }
...
public:
    ...
    void on_create()
    {
        // m_mesh = Mesh::create_from_height_map(Mesh::generate_height_map(16));
        m_mesh = Mesh::create_from_height_map(Mesh::generate_height_map(1));

        m_projection_matrix = Matrix4x4::make_projection(90.0f, m_width / m_height, 0.1f, 1000.0f);
        ...
        float elapsed)
    {
        ...
        if (m_mesh.has_value())
        {
            std::println("mesh has {} triangles", m_mesh->triangles().size());

            for (auto tri : m_mesh->triangles())
            {
                std::println("tri: {}, {}, {}; {}, {}, {}; {}, {}, {};", tri.point_at(0).x(), tri.point_at(0).y(), tri.point_at(0).z(), tri.point_at(1).x(), tri.point_at(1).y(), tri.point_at(1).z(), tri.point_at(2).x(), tri.point_at(2).y(), tri.point_at(2).z());
                ...
                const auto dist = Vec4::distance(tri_transformed.point_at(0), m_camera);
                ...
                std::println("tri_transformed: {}, {}, {}; {}, {}, {}; {}, {}, {};", tri_transformed.point_at(0).x(), tri_transformed.point_at(0).y(), tri_transformed.point_at(0).z(), tri_transformed.point_at(1).x(), tri_transformed.point_at(1).y(), tri_transformed.point_at(1).z(), tri_transformed.point_at(2).x(), tri_transformed.point_at(2).y(), tri_transformed.point_at(2).z());
                ...
                if (Vec4::dot_product(normal, camera_ray) < 0.0f)
                {
                    ...
                    std::println("tri_viewed: {}, {}, {}; {}, {}, {}; {}, {}, {};", tri_viewed.point_at(0).x(), tri_viewed.point_at(0).y(), tri_viewed.point_at(0).z(), tri_viewed.point_at(1).x(), tri_viewed.point_at(1).y(), tri_viewed.point_at(1).z(), tri_viewed.point_at(2).x(), tri_viewed.point_at(2).y(), tri_viewed.point_at(2).z());
                    ...
                    for (auto n = 0; n < clipped_triangles; n++)
                    {
                        ...
                        tri_projected.set_x_on_point_at(0, tri_projected.point_at(0).x() * 0.5f * m_screen_width);
                        tri_projected.set_y_on_point_at(0, tri_projected.point_at(0).y() * 0.5f * m_screen_height);
                        tri_projected.set_x_on_point_at(1, tri_projected.point_at(1).x() * 0.5f * m_screen_width);
                        tri_projected.set_y_on_point_at(1, tri_projected.point_at(1).y() * 0.5f * m_screen_height);
                        tri_projected.set_x_on_point_at(2, tri_projected.point_at(2).x() * 0.5f * m_screen_width);
                        tri_projected.set_y_on_point_at(2, tri_projected.point_at(2).y() * 0.5f * m_screen_height);
        ...
        std::println("triangles to raster: {}", triangles.size());

        for (const auto triangle_to_raster : triangles)
        {
            std::println("tri_to_raster: {}, {}, {}; {}, {}, {}; {}, {}, {};", triangle_to_raster.point_at(0).x(), triangle_to_raster.point_at(0).y(), triangle_to_raster.point_at(0).z(), triangle_to_raster.point_at(1).x(), triangle_to_raster.point_at(1).y(), triangle_to_raster.point_at(1).z(), triangle_to_raster.point_at(2).x(), triangle_to_raster.point_at(2).y(), triangle_to_raster.point_at(2).z());
            ...
            for (const auto it : inner_triangles)
            {
                ...
                SDL_Vertex verts[3] = {
                    SDL_Vertex{SDL_FPoint{it.point_at(0).x(), it.point_at(0).y()}, c, zero_text_coord},
                    SDL_Vertex{SDL_FPoint{it.point_at(1).x(), it.point_at(1).y()}, c, zero_text_coord},
                    SDL_Vertex{SDL_FPoint{it.point_at(2).x(), it.point_at(2).y()}, c, zero_text_coord}};
